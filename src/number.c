...
#define WIDTH 64
#define SIZE_BYTES SIZE*WIDTH/4
...
static int bitstring_to_u64(const char *bitstring, int wordsize, u64 *out) {
  ...
  for (i = 1; i <= wordsize; i++) {
    if (i <= slen)
      num[(SIZE*WIDTH-i)/WIDTH] |= (u64)(bitstring[slen - i] - '0') << (i%WIDTH - 1);
  }
  if (i <= slen) {
#ifdef DEBUG
...
char *number_getHex_u64(number_t *number) {
  ...
  int hp = 2;
  for (; np >= 0; np--) {
    nibble = (0xfULL << (np * 4) & num[(SIZE*WIDTH-hp*4)/WIDTH]) >> (np * 4);
    if (nibble > 15) {
      printf("wtf\n");
      return NULL;
    } else if (nibble > 9)

...
static void item_delete(void *item) { delete_number((number_t *)item); }
...
char *nums_add_string(program_data_t *prog_data, const char *number,
  ...
  while (*chopped != '\0' && (*chopped == '0' || *chopped == 'x'))
    chopped++;
  if (strlen(chopped) < 1)
    strcpy(key, "0");
  else {
    ...
    if ((num = hashtable_find(prog_data->nums, key)) == NULL)
      hashtable_insert(prog_data->nums, key,
                       new_number(type, chopped, prog_data->wordsize));
      ...
      printf("%s already in ht and wordsize is same\n", key);
#endif
  }
...
char *ten_bit_nums_add_string(program_data_t *prog_data, const char *decimal) {
  ...
  if ((num = hashtable_find(prog_data->ten_bit_nums, key)) == NULL)
    hashtable_insert(prog_data->ten_bit_nums, key,
                     new_number(DECIMAL, decimal, 10));
  return key;
}
